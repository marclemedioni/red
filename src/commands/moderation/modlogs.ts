...
import { MessageEmbed, TextChannel, RichEmbed } from 'discord.js';
...
export default class ModLogsCommand extends Command {
    ...
    public async run(msg: CommandMessage, { shouldEnable, msgChannel }: ModLogsArgs) {
        try {
            ...
            msg.client.provider.set(msg.guild, 'modlogs', shouldEnable);

            if (this.isChannel(msgChannel)) {
                msg.client.provider.set(msg.guild, 'modlogchannel', msgChannel.id);
                modlogsEmbed.description += `\n${shouldEnable ? `**Channel:** <#${msgChannel.id}>` : ''}`;
            }

            if (msg.client.provider.get(msg.guild, 'modlogs', true)) {
                logModMessage(
                    msg, msg.client, modlogChannel, msg.guild.channels.get(modlogChannel) as TextChannel, modlogsEmbed
                );
            }
        ...
        **Error Message:** ${err}`);
        ...
        Join the support server by getting an invite by using the \`${msg.client.commandPrefix}invite\` command`);
        }
    }
